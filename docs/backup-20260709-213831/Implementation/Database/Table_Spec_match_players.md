...
- Cada linha representa um atleta relacionado para um quadro específico daquela partida por um time específico.
- `is_team_player = true`
...
- Se o mesmo atleta jogar o segundo quadro e também o primeiro quadro:
  - devem existir duas linhas distintas em `match_players`.
...
- Deve existir no máximo um registro por `match_id + team_id + player_id + frame_type`.
- `shirt_number` não deve duplicar dentro do mesmo `match_id + team_id + frame_type`, salvo regra futura explícita.
