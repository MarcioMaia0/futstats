...
- Registro criado com defaults no cadastro; `profile_visibility` default `PUBLIC` (perfil aberto a todos).
- `profile_visibility` controla o acesso ao perfil (aplicado via RLS); `name_display_*` controla a apresentação do nome dentro da audiência permitida.
...
- Precedência ao resolver qual nome exibir a um viewer (do mais específico ao mais amplo): `TEAM_MEMBERS` → `FOLLOWERS` → `PUBLIC`.
- Se um `name_display_*` indicar `NICKNAME`/`BOTH` mas `users.nickname` for nulo, cai para `NAME`.

## Resolução de nome

- `profile_visibility` (acesso) é aplicado no banco via RLS.
- O rótulo de nome (`name_display_*`) é resolvido **no banco**, por uma função/view em SQL padrão (ex.: `resolve_display_name(viewer_id, profile_id)`), em lote e deduplicada por autor. O cliente apenas formata (`BOTH` → "Nome 'Apelido'").
- Portabilidade: a função recebe o `viewer_id` como parâmetro e usa apenas SQL padrão — sem helpers específicos do Supabase (ex.: `auth.uid()`) dentro da lógica. O acoplamento ao provedor (obter o id do usuário atual) fica na borda (RLS/API), não na resolução. Migrar para outro Postgres é levar a função no dump; para outro banco, reescrever uma função pequena.
